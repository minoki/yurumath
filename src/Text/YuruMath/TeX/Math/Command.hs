...
instance (Monad m, MonadTeXState state m, MonadError String m, IsMathState state) => DoExecute MathCommands m where
  ...
  doGlobal _               = Nothing
  doAdvance Mfam           = Just $ runArithmetic $ advanceInteger famParam
  ...
  doAdvance _              = Nothing
  doMultiply Mfam          = Just $ runArithmetic $ multiplyInteger famParam
  ...
  doDivide Mfam            = Just $ runArithmetic $ divideInteger famParam
